...
confidence_score_ridgeplot <- function(confidence_scores = NULL) {
  ...
  confidence_scores %>%
    ...
    ggplot2::labs(
      title = "Density Plot of Aggregated Confidence Scores",
...
confidence_score_heatmap <- function(
  ...
  if (number_of_claims <= 10) {
    cli::cli_alert_warning(
      "Claims less than ~ 10 might not display effectively."
    )
  }
  ...
  if (number_of_methods <= 5) {
    cli::cli_alert_warning(
      "Provide more than 5 methods for improved contrasting plot"
    )
  }
...
method_types <- function(conf_scores) {
  ...
  conf_scores <- conf_scores %>%
    dplyr::mutate(
      type = dplyr::case_when(
        method %in% NWL ~ "Non-weighted Linear Combintation",
  ...
  conf_scores$type <- factor(
    conf_scores$type,
    levels = c(
      "Non-weighted Linear Combintation",
