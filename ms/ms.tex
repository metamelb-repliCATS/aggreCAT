...
\hypersetup{
  ...
  pdfauthor={Elliot Gould; Charles Gray; Aaron Willcox; Rose O'Dea; Rebecca Groenewegen; David P. Wilkinson},
...
\Plainauthor{Elliot Gould\footnote{School of Ecosystem and Forest
  Sciences, University of Melbourne}Charles GrayAaron WillcoxRose
...
However, from the perspective of the \pkg{aggreCAT} package user it is
...
# A tibble: 2 x 4
  method     paper_id    cs n_experts
  <chr>      <chr>    <dbl>     <int>
1 IndIntWAgg czttvy    74           5
2 IntWAgg    czttvy    74.8         5
\end{verbatim}
...
claims included in the \texttt{focal\_claims} dataframe: czttvy, 65x6p7,
farimg, 09xkh8. However, if we were to supply assessments for only two
claims to \fct{BayesianWAGG}, then we would observe a different result
for focal claim \texttt{czttvy}:
...
{\centering \includegraphics{ms_files/figure-pdf/fig-ridgeplot-1.pdf}
