...
\section{Introducing the aggreCAT package}\label{sec-introduction}
...
example, \pkg{opera}, which generates time-series predictions (CITE).

In this paper we present the \pkg{aggreCAT} package, which was developed
by the repliCATS (Collaborative Assessment for Trustworthy Science)
project \citep{Fraser:2021} as part of the DARPA SCORE (Systematizing
Confidence in Open Research and Evidence) program
\citep{alipourfard2021}, which aimed to generate quantitative
...
effort. The repliCATS project generated Confidence Scores for \(> 4000\)
claims using structured expert elicitation via the IDEA protocol
\citep{hemming2017}, implemented on the repliCATS platform
\citep{Pearson2021}. A subset of claims were randomly selected for
direct replication, against which the elicited Confidence Scores were
ground-truthed \citep{Hanea2021}.

agg package: which provides 27 different methods for mathematically
aggregating judgements within groups of experts into a single forecast.

We aim to provide a detailed overview of the \pkg{aggreCAT} package so
that researchers may apply any of the the aggregation functions
described in \citep{Hanea2021} to their own expert elicitation datasets
where mathematical aggregation is required. Note that judgements that
have already been subjected to behavioural or consensus aggregation may
not be subsequently mathematically aggregated, however individual
elicited judgements may be aggregated mathematically as an alternative
or complement to behavioural or consensus-based aggregation.
...
Before outlining key aggregation methods, we briefly summarise package
datasets, which were collected by the repliCATS project.
...
\citep{hemming2017}, a structured elicitation procedure that draws on
the `wisdom of crowds' to elicit probabilistic judgements from groups of
experts about uncertain events.

The IDEA protocol structures elicitation across two rounds: individuals
first provide judgements independently (\emph{Round 1}), before
anonymously reviewing one another's estimates and engaging in group
discussion, after which they provide a revised set of judgements
(\emph{Round 2}). In each round, participants provide probabilistic
three-point judgements --- a best estimate and upper and lower bounds
--- alongside supplementary ratings of plausibility, comprehensibility,
and involvement in the original study.
...
require only the post-discussion (\emph{Round 2}) three-point
...
method in \textbf{?@tbl-method-summary-table}. Although the aggregation
methods were developed in the context of the IDEA protocol, all methods
requiring only a single round of elicitation can be applied to any
structured elicitation procedure that generates lower, upper, and best
estimates from each individual without enforcing behavioural consensus.

The IDEA (`Investigate', `Discuss', `Estimate', `Aggregate') protocol
structures elicitation across two rounds \citep{hemming2017}:
individuals first provide judgements independently, before anonymously
reviewing one another's estimates and engaging in group discussion,
after which they provide a revised set of judgements. In each round,
participants provide probabilistic three-point judgements --- a best
estimate and upper and lower bounds --- alongside supplementary ratings
of plausibility, comprehensibility, and involvement in the original
study. This elicitation structure directly determines which aggregation
methods in \pkg{aggreCAT} are applicable to a given dataset: most
methods require only the post-discussion three-point judgements, some
additionally use the pre-discussion judgements, and a small number
require supplementary ratings or externally collected data.
...
\subsection*{Box 1: The IDEA Protocol}\label{box-1-the-idea-protocol}
\addcontentsline{toc}{subsection}{Box 1: The IDEA Protocol}

During the \emph{Investigate} phase, individuals review the claim and
draw on background knowledge to provide estimates of the probability of
the claim replicating. These estimates are numeric (0 - 100\%) and
include a best estimate, and upper and lower bounds on that estimate.
...
In the \emph{Discuss} phase, three-point estimates from each group
member are anonymously presented to the group, who then collectively
discuss differences in opinion and provide potential evidence for these
differences. Next, in the \emph{Estimate} phase, individuals are asked
to provide a second set of three-point probablistic judgements. Thus,
for a single assessment, 2 sets of judgements are elicited from each
expert, \emph{pre-} and \emph{post-}group discussion, referred to here
on as \emph{Round 1} and \emph{Round 2} judgements, respectively.

During the fourth phase, \emph{Aggregate}, judgements are mathematically
...
deliver Confidence Scores for over 4000 SBS research claims as a part of
the DARPA SCORE project.

\begin{figure}[H]

{\centering \pandocbounded{\includegraphics[keepaspectratio,alt={The IDEA protocol as deployed by the repliCATS project (reproduced with permission from Wintle et al. 2021).}]{images/img_IDEA_repliCATS.png}}
...
judgements from the repliCATS IDEA protocol \hyperref[fig1]{Figure 1}),
unless appended by a ``1'', where they are \texttt{round\ 1} judgements,
e.g.~\(B1_{i,c}\) denotes the \texttt{round\ 1} Best estimate from
individual \(i\) for claim \(c\).
...
\begin{equation}\protect\phantomsection\label{eq-eqn1}{
...
The \pkg{aggreCAT} package includes several datasets collected by the
...
the IDEA protocol \citep{Hanea2021}.

The core dataset \texttt{data\_ratings} is a tidy \class{data.frame} of
...
elicitation. Additional datasets \citep{Fraser:2021} capture
...
model-derived Bayesian priors (\texttt{data\_supp\_priors}). Known
replication outcomes sourced from previous large-scale replication
projects
\citep{Klein2014, Klein2018ManyL2, Ebersole2016, Camerer2018, aac4716}
for a subset of claims are provided in \texttt{data\_outcomes}.

Full descriptions of each dataset, including variable definitions, data
structure, and example data, are provided in the \pkg{aggreCAT} package
documentation (\texttt{?data\_ratings}) and in the Package Datasets
vignette available on the \pkg{aggreCAT} pkgdown website.

\subsection{`Tidy' Aggregation and Required
Inputs}\label{tidy-aggregation-and-required-inputs}

The design philosophy of \pkg{aggreCAT} is principled on `tidy' data
\citep{Wickham:2014vp}. Each aggregation method expects a
\class{data.frame} or \class{tibble} of judgements as its input, and
returns a \class{tibble} containing the variables \texttt{method},
\texttt{paper\_id}, \texttt{cs} and \texttt{n\_experts} (see
Section~\ref{sec-AverageWAgg} for illustration of outputs); where
\texttt{method} is a character vector corresponding to the aggregation
method name specified in the \texttt{type} argument. Each aggregation is
applied as a summary function \citep{Wickham2017R}, and therefore
returns a single row with a single confidence score \texttt{cs} for each
claim or \texttt{paper\_id}. The number of expert judgements summarised
in the aggregated confidence score is returned in \texttt{n\_experts}.
Because of the tidy nature of the aggregation outputs, multiple
aggregation methods can be applied to the same data with the results of
all aggregation methods row-bound together in a single \texttt{tibble}
(see the example repliCATS workflow in Section~\ref{sec-workflow}).

Every aggregation function requires at minimum the three-point
probabilistic judgements (\texttt{question\ ==\ "direct\_replication"}),
however some methods require only best estimates
(\texttt{element\ ==\ "three\_point\_best"}), and some require multiple
rounds of elicitation. Only the aggregation method \emph{CompWAgg}
additionally requires comprehensibility ratings
(\texttt{question\ ==\ "comprehension"}). For a full summary of each
aggregation method, its calling function, and data requirements, see
\textbf{?@tbl-method-summary-table}.
...
\texttt{focal\_claims} contains 220 rows across four claims and five
participants. Each row records a single \texttt{element} of a
participant's assessment for a given \texttt{paper\_id} and
\texttt{round}. The \texttt{question} column distinguishes the
three-point probabilistic judgements about replicability
(\texttt{direct\_replication}, with \texttt{element} values
\texttt{three\_point\_lower}, \texttt{three\_point\_best}, and
\texttt{three\_point\_upper}) from supplementary assessment data:
participants' plausibility ratings (\texttt{belief\_binary}),
comprehensibility ratings (\texttt{comprehension}), and involvement in
the original study (\texttt{involved\_binary}). All aggregation
functions require at minimum the \texttt{direct\_replication} rows, and
expect data in this long, tidy format.

We will work with data from a subset of participants from a single claim
\texttt{108} for a subset of participants who assessed this claim
(Table~\ref{tbl-focal-claim}).
...
\caption{\label{tbl-focal-claim}Focal Claim Data: Round 2 expert
judgements for claim 108 derived from a subset of 5 claims and 5
participants from \texttt{data~ratings}. Judgements are displayed as
percentages.}
...
\hyperref[aggWorkflow]{Box 1} for details). \texttt{ArMean} (
...
Below we demonstrate the application of \texttt{ArMean} on claim
\texttt{108} (Table~\ref{tbl-focal-claim}). We also illustrate this
aggregation visually in \hyperref[fig-ArMean]{Figure 2}. \texttt{ArMean}
is applied using the aggregation method \fct{AverageWAgg}, which is a
wrapper function for several aggregation methods that calculate
different types of averaged best-estimates (see \texttt{?AverageWAgg}).
...
\texttt{name}. \textless br\textgreater{}
...
\texttt{expert\_judgements} argument of aggregation functions. By
overriding the default value for the argument \texttt{percent\_toggle}
with \texttt{TRUE}, percentage values are converted to probabilities by
dividing judgements over 100 within the aggregation functions.
...
# A tibble: 75 x 5
   ...
   <chr>   <chr>    <chr>      <chr>             <dbl>
 1 round_2 102      7l8m7dmjdb three_point_best     35
 2 round_2 102      7l8m7dmjdb three_point_upper    44
 3 round_2 102      7l8m7dmjdb three_point_lower    30
 4 round_2 102      bvpgcjm09w three_point_lower     5
 5 round_2 102      bvpgcjm09w three_point_upper    30
 6 round_2 102      bvpgcjm09w three_point_best     15
 7 round_2 102      mnlww4qlq3 three_point_best     35
 8 round_2 102      mnlww4qlq3 three_point_upper    55
 9 round_2 102      mnlww4qlq3 three_point_lower     5
10 round_2 102      r7kn2x3mnj three_point_lower     5
# i 65 more rows
\end{verbatim}
...
\includegraphics[width=7.18in,height=\textheight,keepaspectratio]{images/IntervalWAgg.png}
...
(\textbf{?@sec-reasonwagg-supplementary-data}) to compute a vector of
weights, \(w\_reason_{i,c}\) , the number of unique reasons provided by
individual \(i\) in support of their estimate for claim \(c\)
(\hyperref[fig-ReasonWAgg]{Figure 4}). Weights are then normalised
across individuals, multiplied by the Best Estimates for that claim
\(B_{i,c}\) and weighted best estimates are then summed to generate the
Confidence Score (Equation~\ref{eq-ReasonWAgg}).

\begin{figure}

\centering{

\includegraphics[width=5.82in,height=\textheight,keepaspectratio]{images/ReasonWAgg.png}
...
\texttt{ReasonWAgg} requires supplementary data following the format of
\texttt{data\_supp\_reasons}, in which each row records whether a given
participant (\texttt{user\_name}) invoked each of the 25 qualitatively
coded reasoning categories \citep{Wintle:2021} in support of their
estimate for a given claim (\texttt{paper\_id}). Column names are
prefixed with \texttt{RW}. We prepare this data by subsetting to the
participants in \texttt{focal\_claims}, and illustrate the structure for
focal claim \texttt{24} below (Shown in
Table~\ref{tbl-reason-wagg-focal-claim}, see
\texttt{?data\_supp\_reasons} for full variable definitions):
...
\caption{\label{tbl-reason-wagg-focal-claim}Focal Claim \(24\)
judgements comprising best estimates, upper and lower bounds elicited
from five participants. Judgements are displayed as percentages.}
...
\fct{BayesianWAgg} using the argument \texttt{priors}, where
\texttt{data\_supp\_priors} provides a prior probability of replication
on the logit scale (\texttt{prior\_means}) for each claim
(\texttt{paper\_id}).
...
argument \texttt{percent\_toggle} (\hyperref[aggWorkflow]{Box 1}):
...
\includegraphics[width=9.375in,height=\textheight,keepaspectratio]{images/BayesianWAgg.png}
...
\citep[see \citet{lst-multi-method-workflow-non-supp} and
\citet{lst-multi-method-workflow-both}]{purrr2020}.
...
ArMean & 1 & 0.099 & 1.00 \\
BayTriVar & 1 & 0.115 & 1.00 \\
IndIntWAgg & 1 & 0.099 & 1.00 \\
IntWAgg & 1 & 0.091 & 1.00 \\
ReasonWAgg & 1 & 0.088 & 1.00 \\
ShiftWAgg & 1 & 0.132 & 0.75 \\
\end{tblr}

}

\caption{\label{tbl-multi-method-workflow-eval}AUC and Classification
Accuracy for forecasts from the aggregation methods `ShiftWAgg',
`ArMean', `IntWAgg', `IndIntWAgg', `ReasonWAgg' and `BayTriVar' for a
subset of the repliCATS pilot study claims (\texttt{focal\_claims}) and
known outcomes.}
...
\pandocbounded{\includegraphics[keepaspectratio]{aggreCAT_files/figure-pdf/fig-aggregation-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{aggreCAT_files/figure-pdf/fig-heatmap-1.pdf}}
...
For example, in Figure~\ref{fig-heatmap}, for the dataset
...
(e.g.~\texttt{ReasonWAgg} for claims \texttt{109} and \texttt{138}.
...
\hyperref[aggWorkflow]{1}), and that the expert judgements are elicited
using the appropriate method, as required by each aggregation method
(see Table \ref{tbl-method-summary-table}).
...
\caption{\label{lst-multi-method-workflow-both}If we wish to batch
aggregate claims using a combination of aggregation methods that do and
do not require supplementary data, we must aggregate them separately,
since the methods that require supplementary data have an additional
argument for the supplementary data that must be parsed to the wrapper
function call. We can chain the aggregation of the methods that do not
require supplementary data, and the methods that do require
supplementary data together very neatly using \pkg{dplyr}'s
\fct{bind_rows} function \citep{dplyr2021} and the \fct{magrittr} pipe
\texttt{\%\textgreater{}\%} \citep{magrittr2020}. Below we implement
this approach while applying the aggregation methods \texttt{ArMean},
...
- Session info ---------------------------------------------------------------
 ...
 quarto   1.9.27 @ /Applications/quarto/bin/quarto

- Packages -------------------------------------------------------------------
 package      * version  date (UTC) lib source
 abind          1.4-8    2024-09-12 [1] CRAN (R 4.5.0)
 aggreCAT     * 1.0.0    2025-05-28 [1] CRAN (R 4.5.0)
