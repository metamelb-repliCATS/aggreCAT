...
into a single prediction. Aggregation becomes more complicated when
judgements are elicited with uncertainty bounds and when there are
multiple rounds of elicitation. This paper presents the new \proglang{R}
...
management \citep{Sutherland2018}. It is best-practice to elicit
judgements from diverse groups to capitalise on the `wisdom of crowds'
\citep{hemming2017}, because groups of experts tend to perform better
than a single expert \citep{Goossens2008}. However judgements or
forecasts must then be distilled into a single forecast, ideally
accompanied by estimates of uncertainty around those estimates
\citep{Hanea2021}. Judgements from multiple experts may be combined into
a single forecast using either behavioural approaches that force experts
into forming consensus, or mathematical approaches \citep{Goossens2008}.
...
aggregated Confidence Scores are `ground-truthed' or verified. The aim
of the DARPA SCORE project is to aid the development of artificial
...
\item
  ...
  may be elicited using the IDEA protocol or another similarly
...
\item
  ...
  questions, not just the target claim being aggregated. Secondly, for
  aggregation methods that calculate variance in estimates, variance
  cannot be calculated on a single data point. While two is the
...
claims are claims where the replication study found a statistically
...
responses are contained in \texttt{data\_supp\_quiz} and are used to
construct performance weights for the aggregation method
...
\texttt{tibble} (See the example repliCATS workflow in
Section~\ref{sec-workflow}).

The tibble of judgements to be aggregated (\texttt{data\_ratings})
requires the columns \texttt{round}, \texttt{paper\_id},
\texttt{user\_name}, \texttt{question}, \texttt{element}, \texttt{value}
and \texttt{group}. Each observation in the judgement data corresponds
to a single \texttt{value} for a single \texttt{question} elicited from
a single \texttt{user\_name} about a given \texttt{paper\_id} in a
single \texttt{round}. There are four types of \texttt{question}s that
elicited \texttt{values} correspond to. Estimates about the event
probability for a given \texttt{paper\_id} correspond to
\texttt{"direct\_replication"} in the \texttt{question} variable. The
type of estimate the \texttt{value} belongs to is recorded in the
\texttt{element} variable, and may be one of
\texttt{"three\_point\_lower"}, \texttt{"three\_point\_best"}, or
\texttt{"three\_point\_upper"}.

Every aggregation function requires at least one \texttt{value} derived
from three-point elicitation
(\texttt{question\ ==\ "direct\_replication"}) in the dataframe supplied
to the \texttt{expert\_judgements} argument, however, some methods
require only the best-estimates
(\texttt{element\ ==\ "three\_point\_best"}) for mathematical
aggregation. Similarly some aggregation methods require multiple
\texttt{round}s of judgements, while others require only a single round.
Only the aggregation method \emph{CompWAgg} requires \texttt{value}s for
the \texttt{comprehension} question. For a summary of each aggregation
method, its calling function and data requirements and sources, see
...
\toprule
Claim ID & User Name & Lower Bound & Best Estimate & Upper Bound \\ \midrule %% TinyTableHeader
...
\begin{equation}\phantomsection\label{eq-ArMean}{
...
\hyperref[fig-ArMean]{Figure 2}. \texttt{ArMean} is applied using the
aggregation method \fct{AverageWAgg}, which is a wrapper function for
several aggregation methods that calculate different types of averaged
best-estimates (see \texttt{?AverageWAgg}). The function returns the
Confidence Score for the claim in the form of a \class{tibble}:
...
\includegraphics[width=6.25in,height=\textheight]{images/ArMean.png}
...
\begin{tcolorbox}[enhanced jigsaw, colback=white, breakable, colframe=quarto-callout-color-frame, leftrule=.75mm, opacityback=0, rightrule=.15mm, arc=.35mm, toprule=.15mm, left=2mm, bottomrule=.15mm]

\phantomsection\label{aggWorkflow}
...
associated values. Nearly all aggregation functions use only the round
two judgements, so the \texttt{round\_2\_filter} defaults to
\texttt{TRUE} (See Table \ref{tbl-method-summary-table} for required
inputs of all aggregation methods). \texttt{preprocess\_judgements()}
further pre-processes the data to remove missing data, and returns the
data into an appropriate structure for calculating weights and applying
the aggregation function with \texttt{dplyr::summarise()}.
...
estimates, \(B_{i,c}\), and summed together into a single Confidence
Score (Figure~\ref{fig-IntWAgg-IndIntWAgg}).
...
\begin{equation}\phantomsection\label{eq-weightnIndivInterval}{
w\_Interval_{i,c}= \frac{1}{U_{i,c} - L_{i,c}}
}\end{equation}

\begin{equation}\phantomsection\label{eq-IntWAgg}{
...
\begin{equation}\phantomsection\label{eq-ReasonWAgg}{
...
\texttt{24}, and the round two three-point estimates from the five focal
...
\toprule
Claim ID & User Name & round & Lower Bound & Best Estimate & Upper Bound \\ \midrule %% TinyTableHeader
...
fact, the Bayesian methods require data from more than a single claim in
order for the function to properly execute, due to the underlying
mathematical specification of the models (See \texttt{?BayesianWAgg} and
below for details).
...
to the \texttt{expert\_judgements} argument. Because data from claims
than the target claim are used to calculate the Confidence Score for the
target claim, any data from claims other than the target claim within
the data parsed to the \texttt{expert\_judgements} argument in
\emph{BayesianWAgg} will alter the Confidence Score. Above, we
calculated the Confidence Score for claim \texttt{108} but including
information from three additional claims included in the
\texttt{focal\_claims} \class{data.frame}: \texttt{108}, \texttt{138},
\texttt{186}, \texttt{24}. However, if we were to supply assessments for
only two claims to \fct{BayesianWAgg}, then we would observe a different
result for focal claim \texttt{108}:
...
methods. Methods that \emph{do} require the full IDEA protocol for their
correct mathematical implementation are listed in Table
\ref{tbl-method-summary-table}, e.g.~\emph{ShiftingWAgg}, which uses two
...
\includegraphics[width=9.375in,height=\textheight]{images/BayesianWAgg.png}
...
workflow for using the \pkg{aggreCAT} package to aggregate expert
...
processing and application of the aggregation methods is handled
...
\texttt{data\_outcomes} dataset published with \pkg{aggreCAT}. In this
dataset, each claim \texttt{paper\_id} is assigned an \texttt{outcome}
of \texttt{0} if the claim did not replicate and \texttt{1} if the claim
...
\toprule
Method & AUC & Brier Score & Classification Accuracy ($\%$) \\ \midrule %% TinyTableHeader
ArMean & 1 & 0.099 & 100 \\
BayTriVar & 1 & 0.115 & 100 \\
IndIntWAgg & 1 & 0.099 & 100 \\
IntWAgg & 1 & 0.091 & 100 \\
ReasonWAgg & 1 & 0.088 & 100 \\
ShiftWAgg & 1 & 0.132 &  75 \\
\bottomrule
\end{tblr}
...
points, and their upper and lower bounds displayed as black error bars.
...
\caption{\label{fig-heatmap}Blocked heatmap of confidence scores is
useful for visually comparing aggregation methods and evaluating them
against a set of known outcomes. In this example, Confidence Scores
generated by six aggregation methods for the repliCATS pilot study are
presented for 25 claims. Claims where known outcomes succesfully
replicated \texttt{outcome\ ==\ TRUE} are presented in heatmap on the
left, and claims that failed to replicate are presented in heatmap on
the right. Confidence Scores generated by different aggregation methods
are positioned along the y-axis, with vertical groupings according to
the methods' mathematical properties. Colour and intensity of cells
indicates the direction and degree of deviation respectively of the
Confidence Scores from the known outcomes.}
...
\toprule
Method & Question ID & Confidence Score & N (experts) \\ \midrule %% TinyTableHeader
ArMean & 1 & 141.67 & 5 \\
...
methods return \class{data.frame}s of aggregated forecasts. There are
four key benefits of the aggreCAT package. Firstly, the work of
data-wrangling and application of the aggregation methods is handled
internally by the aggregation methods, so that the researcher can focus
on analysis and interpretation of the aggregation outputs. This is
critical in data-deficient contexts where rapid assessments are needed,
which is a common use-case for the use of expert derived forecasts.
Secondly, the \pkg{aggreCAT} package is easily paired with other
tidyverse tools, such as \pkg{purrr}, \pkg{dplyr}, and \pkg{ggplot2}, as
exemplified through the repliCATS workflow described in
Section~\ref{sec-workflow}.
...
be lodged on the \pkg{aggreCAT}
\href{https://github.com/metamelb-repliCATS/aggreCAT}{GitHub repository}
using reproducible examples created with \pkg{reprex}
\citep{reprexpkg2020} on the repliCATS pilot study datasets shipped with
the \pkg{aggreCAT} package.
...
- Packages -------------------------------------------------------------------
 ...
 aggreCAT     * 0.0.0.9004 2024-01-31 [1] Github (metamelb-repliCATS/aggreCAT@773617e)
 ...
 DescTools      0.99.53    2024-01-18 [1] CRAN (R 4.3.1)
 ...
 precrec        0.14.4     2023-10-11 [1] CRAN (R 4.3.1)
 profvis        0.3.8      2023-05-02 [1] CRAN (R 4.3.0)
 promises       1.2.1      2023-08-10 [1] CRAN (R 4.3.0)
 proxy          0.4-27     2022-06-09 [1] CRAN (R 4.3.0)
 purrr        * 1.0.2      2023-08-10 [1] CRAN (R 4.3.0)
 R2jags         0.7-1      2021-08-05 [1] CRAN (R 4.3.0)
 R2WinBUGS      2.1-22     2023-12-16 [1] CRAN (R 4.3.1)
 ...
 rfUtilities    2.1-5      2019-10-03 [1] CRAN (R 4.3.0)
 rjags          4-15       2023-11-30 [1] CRAN (R 4.3.2)
 ...
 tinytable    * 0.0.2      2024-01-26 [1] CRAN (R 4.3.1)
