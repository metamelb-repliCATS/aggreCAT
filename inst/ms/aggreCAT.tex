...
this paper we present the \pkg{aggreCAT} package, which provides
...
by the repliCATS project \citet{Fraser:2021}. \textbf{X} of these
...
\texttt{data\_justifications}. Finally, \pkg{aggreCAT} contains three
...
We illustrate this using a single claim \texttt{zttvyg} for a subset of
5 out of 25 participants from the \texttt{data\_ratings} dataset. Note
...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, rightrule=.15mm, toprule=.15mm, breakable, arc=.35mm, bottomrule=.15mm, colback=white, left=2mm, leftrule=.75mm, colframe=quarto-callout-color-frame]
...
\pkg{purrr} function \fct{map_dfr}, which row-binds the results of each
...
but not contained in \texttt{data\_ratings}, see Table
\ref{tbl-method-summary-table} for details.
...
that use supplementary data (\texttt{data~supp~ReasonWAgg}) collected
...
supplementary data to the {reasons} argument:
...
\fct{BayesianWAgg} using the argument {priors}
...
percentages within the data parsed to {expert\_judgements} to
probabilities, the logical value \texttt{TRUE} is supplied to the
argument {percent\_toggle} (\protect\hyperlink{aggWorkflow}{Box 1}):
...
other claims and participants included in the {expert\_judgements}
argument other than the target claim, because, by definition,
\fct{BayesianWAgg} calculates the Confidence Score for a target claim
using data from participants' assessments of other claims, and from all
other claims in the dataframe parsed to the {expert\_judgements}
argument. Because information about other claims than the target claim
is used to calculate the Confidence Score for the target claim, what is
included in the data supplied to the argument {expert\_judgements} in
\fct{BayesianWAgg} will alter the Confidence Score. Above, we calculated
the Confidence Score for claim \texttt{czttvy} but including information
from 3 additional claims included in the \texttt{focal\_claims}
dataframe: 108, 138, 186, 24. However, if we were to supply assessments
for only two claims to \fct{BayesianWAgg}, then we would observe a
different result for focal claim \texttt{czttvy}:
...
from the \texttt{focal\_claims} data object.}
...
\fct{map_dfr} function \citep{purrr2020} , which takes a list of
...
mapping function. We therefore use \fct{purr::map2_dfr} instead of
\fct{purr::map_dfr} because there are now multiple inputs that must be
...
using \pkg{dplyr}'s \fct{bind_rows} function \citep{dplyr2021} and the
\fct{magrittr} pipe \texttt{\%\textbackslash{}\textgreater{}\%}
...
generates ridgeline plots using \pkg{ggridges} \citet{ggridges2021} ,
and displays the distribution of predicted outcomes across a suite of
...
provided to the argument {confidence\_scores} organised with unique
aggregation methods on the y-axis, and separate forecasts or
...
groups, according to the binary outcomes in {data\_outcomes}.
...
\fct{preprocess_judgements} on {focal\_claims}, transforming judgements
in percentages to probabilities by setting {percent\_toggle} to
\texttt{TRUE}, Figure~\ref{fig-aggregation},
...
aggregation method (see Table \ref{tbl-method-summary-table}).
...
{percent\_toggle} argument for each aggregation wrapper function to
{FALSE}: (DAVID/AAron to check.. )
...
\begin{longtable}[t]{>{\raggedright\arraybackslash}p{10em}>{\raggedright\arraybackslash}p{20em}l>{\raggedright\arraybackslash}p{10em}>{\raggedright\arraybackslash}p{5em}>{\raggedright\arraybackslash}p{10em}>{\raggedright\arraybackslash}p{10em}}
...
\hspace{1em}?EngWAgg & Weighted by the level of engagement as measured by the individuals' verbosity & `data_justifications` & Calculated internally & 2 & Single-Point & ${B}_{i,c}$, $w$ {\_}${Eng}_{i,c}$\\
\hspace{1em}?CompWAgg & Weighted by the level of self rated comprehension of the claim the individuals' report &  & Calculated internally & 1 & Single-Point & ${B}_{i,c}$\\*
\end{longtable}
\endgroup{}
...
- Packages ---------------------------------------------------------
 ...
 aggreCAT      * 0.0.0.9002 2022-11-01 [1] local
 ...
 insight         0.18.6     2022-10-23 [1] CRAN (R 4.2.0)
 ...
 rjags           4-13       2022-04-19 [1] CRAN (R 4.2.1)
