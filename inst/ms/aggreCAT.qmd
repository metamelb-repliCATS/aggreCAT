...
`data_ratings` is a *tidy* [data.frame]{.class} wherein each *observation* (or row)
...
\<br\>
Percentage values, counts, or other non probabilistic quantities are the default expected value type for ratings supplied to the `expert_judgements` argument of aggregation functions. By overriding the default value for the argument `percent_toggle` with `TRUE`, percentage values are converted to probabilities by dividing judgements over 100
...
[purrr]{.pkg}'s [map\_dfr]{.fct} function [@purrr2020, however see @lst-multi-method-workflow-non-supp for a row-binding approach].
...
because [aggreCAT]{.pkg} functions return tidy [data.frame]{.class}s, we can easily
