...
In this paper we present the [aggreCAT]{.pkg} package, which was
developed by the repliCATS (Collaborative Assessment for Trustworthy
Science) project [@Fraser:2021] as part of the DARPA SCORE
(Systematizing Confidence in Open Research and Evidence) program
[@alipourfard2021], which aimed to generate quantitative 'Confidence
Scores' — estimates of the likely replicability of research claims from
the social and behavioural sciences — for use as a proxy measure of
credibility in the absence of direct replication effort. The repliCATS
project generated Confidence Scores for $> 4000$ claims using structured
expert elicitation via the IDEA protocol [@hemming2017], implemented on
the repliCATS platform [@Pearson2021]. A subset of claims were randomly
selected for direct replication, against which the elicited Confidence
Scores were ground-truthed [@Hanea2021].

<!-- find segue into next para -->

agg package: which provides 29 different methods for mathematically
aggregating judgements within groups of experts into a single forecast.

This paper aims to provide a detailed overview of the [aggreCAT]{.pkg}
package so that researchers may apply any of the the aggregation
...
context of the IDEA ('Investigate', 'Discuss', 'Estimate', 'Aggregate')
protocol [@hemming2017], a structured elicitation procedure that draws
on the 'wisdom of crowds' to elicit probabilistic judgements from groups
of experts about uncertain events.
...
@tbl-method-summary-table. Although the aggregation methods were
developed in the context of the IDEA protocol, all methods requiring
only a single round of elicitation can be applied to any structured
elicitation procedure that generates lower, upper, and best estimates
from each individual without enforcing behavioural consensus.
...
This elicitation structure directly determines which aggregation methods
in [aggreCAT]{.pkg} are applicable to a given dataset: most methods
require only the post-discussion three-point judgements, some
...
To collect expert judgements about the replicability of SBS claims, we
...
the form of upper and lower bounds on those point-estimates. Judgements
were elicited using the repliCATS platform [@Pearson2021], a multi-user
...
replicability. The repliCATS project developed 29 different methods for
...
where the replication study found a significant result in the same
direction as the original research claim, and '`FALSE`' claims are those
where the replication study *did not* find a significant result in the
same direction as the original study. For each claim $c$, an individual
$i$ assesses the probability of a claim replicating by providing three
probabilities: a lower bound ${L}_{i,c}$, an upper bound ${U}_{i,c}$,
and a best estimate $B_{i,c}$, satisfying the inequalities:
...
remove missing data, and returnq the data into an appropriate structure
...
`ReasonWAgg` requires supplementary data following the format of
`data_supp_reasons`, in which each row records whether a given
participant (`user_name`) invoked each of the 25 qualitatively coded
reasoning categories [@Wintle:2021] in support of their estimate for a
given claim (`paper_id`). Column names are prefixed with `RW`. We
prepare this data by subsetting to the participants in `focal_claims`,
and illustrate the structure for focal claim `24` below (Shown in
@tbl-reason-wagg-focal-claim, see `?data_supp_reasons` for full variable
definitions):
...
Bayesian methods require more than a single claim's worth of data to
work properly execute due mathematical specification of the models (See
`?BayesianWAgg` and below for details).
...
[BayesianWAgg]{.fct} using the argument `priors`, where
`data_supp_priors` provides a prior probability of replication on the
logit scale (`prior_means`) for each claim (`paper_id`).
...
`expert_judgements` argument. Because information about other claims
than the target claim is used to calculate the Confidence Score for the
target claim, what is included in the data supplied to the argument
`expert_judgements` in [BayesianWAgg]{.fct} will alter the Confidence
Score. Above, we calculated the Confidence Score for claim `108` but
...
[BayesianWAgg]{.fct}, then we would observe a different result for focal
claim `108`:
...
```{r}
#| tbl-cap: 'TEST TABLE'
#| label: tbl-method-summary-table-TEST
library(gt)
library(gtExtras)
aggreCAT:::method_summary_table %>%
  ungroup %>%
  ...
  tidyr::unite(agg_name_description,
               ...
               aggregator_fun_desc, sep = " ") %>%
  select(-agg_name_description) %>%
  mutate(supp_data_requirements = tidyr::replace_na(supp_data_requirements, " "))  |> slice(1:3)  |>  gt()  |> gt::fmt_markdown(judgement_data_sources_eqns)  |> gt::cols_label(type ~ "Method",
                             type_desc ~ "Description",
                             supp_data_requirements ~ "Data Requirements",
                             weighting_fn ~ "Weighting Function",
                             number_rounds ~ "Elicitation Rounds",
                             elicitation_method ~ "Elicitation Method",
                             judgement_data_sources_eqns ~ "Data Sources")
```
