...
**table X (cross-ref)**. Before outlining key aggregation methods, we
...
that claim. See section **ref(ReasonWAgg)** for details on the
...
`paper_id`, `cs` and `n_experts` (see section **ref(ArMean)** for
...
using separate weighting functions ([See Table 1](#table1)), however,
...
[percent_toggle]{.arg} **(cross-ref to blueprint box):**
