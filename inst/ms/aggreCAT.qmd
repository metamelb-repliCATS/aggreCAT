...
The [aggreCAT]{.pkg} package includes several datasets collected by the repliCATS project during a pilot study at a two-day workshop in the Netherlands in July 2019, at which 25 participants assessed the replicability of 25 unique SBS research claims with known outcomes using the IDEA protocol [@Hanea2021].

The core dataset data_ratings is a tidy [data.frame]{.class} of probabilistic three-point judgements (lower bound, best estimate, upper bound) elicited from participants for each claim, across two rounds of elicitation. Additional datasets [@Fraser:2021] capture participants' written justifications (`data_justifications`), peer comments and ratings (`data_comments`), and three supplementary datasets collected externally to the IDEA protocol: a statistical knowledge quiz (`data_supp_quiz`), qualitatively coded reasoning categories (`data_supp_reasons`), and model-derived Bayesian priors (`data_supp_priors`). Known replication outcomes sourced from previous large-scale replication projects [@Klein2014; @Klein2018ManyL2; @Ebersole2016; @Camerer2018; @aac4716] for a subset of claims are provided in `data_outcomes`.

Full descriptions of each dataset, including variable definitions, data structure, and example data, are provided in the [aggreCAT]{.pkg} package documentation (`?data_ratings`) and in the Package Datasets vignette available on the [aggreCAT]{.pkg} pkgdown website.
