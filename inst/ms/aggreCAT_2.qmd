...
<!-- #TODO not sure where to put the following: -->
...
<!-- ? duplicated content abouve types of weights, which one should be removed? -->
...
`IntervalWAgg()` wrapper function implements several methods that calculate different types of weighted linear combinations of best estimates with weights calculated from uncertainty bounds (see `?IntervalWAgg`). Let's apply the `IntWAgg` method to the focal claim `108` (@tbl-focal-claim) with the `IntervalWAgg()` wrapper function by specifying `type = "IntWAgg"`:
