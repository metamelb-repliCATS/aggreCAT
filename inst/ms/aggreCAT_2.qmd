...
To account for differences in interval widths across claims, judgement-level weights can be rescaled. In `IndIntWAgg` each participant’s best estimate $B_{i,c}$ is weighted by their interval width for claim $c$ relative to their average interval width across all claims. When the interval width for a given claim is narrower than the participant's average interval width across claims, it implies greater confidence and potentially better forecasting performance.
