...
`weights` are constructed from the `data_supp_quiz` dataset,  which is a dataset of quiz scores for each participant in the repliCATS pilot study (See `?data_supp_quiz` for details about the dataset). The column containing the quiz scores is renamed to `weight`, as required by `LinearWAgg()`. The weights are then normalised across participants, multiplied by $B_{i,c}$, and summed to give the aggregated confidence score. We specify the `name` of the method to be returned in the output as "QuizWAgg":
