...
`focal_claims` contains `r focal_claims  |> nrow()` rows across four claims and five
...
unweighted aggregation wrapper, `ArMean()`. Below we apply the arithmetic mean method   to the focal claim `108` (@tbl-focal-claim) using the [*AverageWAgg*]{.fct} wrapper function, which implements several methods that calculate different types of averaged best-estimates (see `?AverageWAgg`). The `type` argument specifies the method of averaging, and in this case, we specify `type = "ArMean"` to apply the `ArMean`. The output is a [tibble]{.class} containing the method name, paper ID, confidence score, and number of experts whose judgements were aggregated.
All other aggregation methods take this underlying computational blueprint, and expand on it according
to the aggregation methods' requirements (See Box [2](#aggWorkflow) for details).

`ArMean()` takes the unweighted linear average (i.e. arithmetic mean) of the best estimates, $B_{i,c}$, across all $N$ experts for a given claim $c$ to calculate the aggregated confidence score $\hat{p}_c$ [@eq-ArMean].
...
Other aggregation wrapper functions that do not weight judgements include: `ExtremisationWAgg()`, `DistributionWAgg()`, and `BayesianWAgg()` (see @sec-BayesianWAgg).

<!-- #TODO check the above sentence true?? -->

### Weighted Linear Combination of Judgements

Many aggregation methods in [aggreCAT]{.pkg} use weighting schemes to differentially weight individual judgements according to proxies of forecasting performance. Weights may be calculated at the judgement-level or participant-level. At the judgement-level, weights are calculated from the elicited judgements themselves for each claim. At the participant-level, weights may be calculated by additional information about the individual, such as their performance on a quiz of statistical knowledge, or their average interval width across all claims. In the latter case, participant-level weights are calculated from the elicited judgements themselves but are rescaled across all claims to reflect systematic differences in the elicited judgements across all claims for each individual.

#### Judgement-level Weights

`IntervalWAgg` uses judgement-level weights by generating linearly weighted best estimates with weights influenced by the interval width of the forecast. The `IntWAgg` method implemented by the `IntervalWAgg()` function weights each participant's best estimate $B_{i,c}$ by the width of their uncertainty intervals, i.e. the difference between an individual's upper ${U}_{i,c}$ and lower bounds ${L}_{i,c}$. The intuition behind this weighting scheme is that participants who provide narrower uncertainty intervals are more confident in their estimate, and therefore likely to be more accurate for the target claim. Hence, participants with narrower intervals are weighted more heavily in the aggregation than those with wider intervals.
