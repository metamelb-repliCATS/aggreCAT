...
Each wrapper follows the same pipeline: pre-process judgements, compute weights when needed, aggregate, then post-process results into tidy output. Pre-processing with `pre_process_judgements()` standardises inputs and filters to required rounds, elements, and questions. For example, methods requiring only `round 2` estimates remove other rounds; methods requiring only best estimates retain only that element. By default, single-round methods use `round 2`, and three-point methods use `three_point_best`, unless method-specific requirements or user inputs override this.
...
### User-supplied Weights: Participant-level {#sec-QuizWAgg}
