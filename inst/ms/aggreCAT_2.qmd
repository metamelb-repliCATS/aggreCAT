...
All [aggreCAT]{.pkg} aggregation wrappers share four core arguments: `type` (the aggregation method applied to  expert_judgements`), optional `name` (to override the default method label), `placeholder` (return a fixed confidence score), and `percent_toggle` (convert percentage inputs to probabilities).
